...
export function initBlitz() {
  ...
  async function ensureExploreBase() {
    if (!state.target) {
      ...
      if ($("exploreSub")?.firstChild) $("exploreSub").firstChild.textContent = "No image loaded yet — showing the demo shot. Upload yours in Edit, or ";
      return;
    }
    state.exploreBase = state.target.el;
    if ($("exploreSub")?.firstChild) $("exploreSub").firstChild.textContent = "Every thumbnail below is your shot, graded live in your browser. ";
  }
  ...
  async function buildExplore(force) {
    if (!$("lookGrid")) return;                  // community hidden — nothing to build
