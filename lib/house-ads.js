/* House ads - cross-promotion across John's own properties.
   ...
   Each unit is TWO links, not one: the header line sells the slot, the body
   advertises. Links cannot nest, and a header reading "contact us" that
   navigated somewhere else would be a trap, so they are siblings. */

const SITE = "blitz";              // this site's key - excluded from its own inventory
const MEDIA = "/house-ads/";       // where the two images per key live
const CONTACT = "abel.manoah@gmail.com";
const LABEL = "Contact us for ad opportunities";
const BADGE = "Ad";                // disclosure, following the usual convention

/* Outbound links carry UTM parameters so the receiving site can attribute the
   visit. GA4 reads these automatically, so nothing needs installing at the
   other end. utm_content records which slot was clicked, which is how you
   learn whether the leaderboard or the rail actually earns its space. */
const UTM = {
  utm_source: "blitzluts.com",
  utm_medium: "house_ad",
...
function shapeOf(slot) {
  return slot.dataset.houseAd === "banner" ? "sq" : "wide";
}

/* Append tracking without clobbering anything the destination already sets,
   and never break the link over it: a malformed href returns unchanged rather
   than throwing and leaving the ad unclickable. */
function withTracking(href, slotName) {
  try {
    const u = new URL(href);
    for (const [k, v] of Object.entries(UTM)) {
      if (!u.searchParams.has(k)) u.searchParams.set(k, v);
    }
    if (slotName && !u.searchParams.has("utm_content")) {
      u.searchParams.set("utm_content", slotName);
    }
    return u.toString();
  } catch {
    return href;
...
function render(slot, ad) {
  ...
  const size = shape === "sq" ? "728x90" : "300x250";

  const wrap = document.createElement("div");
  wrap.className = "housead";

  /* Header row: discloses on the left, sells the slot on the right, which is
     where the eye already looks for these on any ad-supported page. */
  const head = document.createElement("div");
  head.className = "housead-head";

  /* Left: the disclosure. Deliberately NOT aria-hidden - a screen reader user
     is owed the same "this is an ad" signal as everyone else. */
  const badge = document.createElement("span");
  badge.className = "housead-badge";
  badge.textContent = BADGE;

  /* Right: a real mailto, because inviting contact and then navigating to an
     advertiser would be a dark pattern. The subject carries the unit size so
     an enquiry says which slot it is about. */
  const enquire = document.createElement("a");
  enquire.className = "housead-label";
  enquire.href = "mailto:" + CONTACT + "?subject=" +
    encodeURIComponent("Advertising on blitzluts.com - " + size);
  enquire.textContent = LABEL;

  head.appendChild(badge);
  head.appendChild(enquire);

  /* Body: the advertisement itself. */
  const a = document.createElement("a");
  a.className = "housead-link";
  a.href = withTracking(ad.href, slot.dataset.houseAd || "");
  a.target = "_blank";
  a.rel = "noopener";
  /* One composed label, because the picture, headline, body and call to
     action are four fragments of a single link. */
  a.setAttribute("aria-label",
    "Advertisement. " + ad.title + ". " + ad.body + " " + ad.cta + ", opens in a new tab.");
  ...
  a.innerHTML =
    '<img class="housead-media" alt="" loading="lazy" decoding="async"' +
      ...
      ' src="' + escapeHtml(MEDIA + ad.key + "-" + shape + ".webp") + '">' +
  ...
  enquire.addEventListener("click", () => {
    if (typeof window !== "undefined" && window.gtag) {
      window.gtag("event", "ad_enquiry_click", { slot: slot.dataset.houseAd || "" });
    }
  });
